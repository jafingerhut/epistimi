...
For the line $y=D$ where $A$ and $B$ are (far above the bounds of the
figure), $A$ is on the line $k=0.27$ at coordinates $(0, D)$.
But note that $B$ is moving to the right at speed $v$ when the pulses
are emitted, so it is some distance to the right of $A$ when $B$
receives the pulses.  Is it so far to the right that it is on the
$k=-0.27$ line?

As we will soon see, it turns out that $B$ reaches far enough that if
you measure the difference in times that it receives the pulses,
according to time in $A$'s frame, and then multiply that by $1/\gamma$
to account for $B$'s clock running slower than $A$'s clock, the result
is $-0.27$ sec.  And this correspondence is true for all $\beta$
values, not only $\beta=0.5$ used in
