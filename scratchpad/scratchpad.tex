...
q_3 - q_2
  ...
  & = \frac{L}{\beta c} (1 - \sqrt{ 1 - \beta^2 }) & & \text{defn. of $\gamma$} \nonumber \\
...
\vect{s} + \vect{r} & = ((-L/2)(1+1/\gamma), D) & & \text{$\vect{s}+\vect{r}$ is location of $B$ at time $t_1=0$} \\
\vect{r} & = (x_0, y_0) = (-L/(2 \gamma), D) & & \text{subtract $\vect{s}$ from previous line to get $\vect{r}$} \\
{x_0}' & = x_0 + v(t_0 - t_1) = L/(2\gamma) & & \text{a little algebra from the above} \\
{r'}^2 & = {{x_0}'}^2 + y_0^2 = \frac{L}{4 \gamma^2} + D^2
\end{align*}
The result of Lemma~\ref{lem:IntersectingSpreadingPulse} gives us the
time (in $R$'s frame) when $B$ receives the event 2 pulse:
\begin{equation}
  e_2 = \frac{L}{\gamma v} +
        \frac{ \frac{Lv}{2\gamma} + \sqrt{ c^2 ( \frac{L^2}{4 \gamma^2} + D^2 ) - v^2 D^2} }{c^2 - v^2}
...
\vect{s} + \vect{r} & = ((-L/2)(1+1/\gamma), D) & & \text{$\vect{s}+\vect{r}$ is location of $B$ at time $t_1=0$, same as above} \\
\vect{r} & = (x_0, y_0) = (-L - L/(2 \gamma), D) & & \text{subtract $\vect{s}$ from previous line to get $\vect{r}$} \\
{x_0}' & = x_0 + v(t_0 - t_1) = -L/(2\gamma) & & \text{a little algebra from the above} \\
{r'}^2 & = {{x_0}'}^2 + y_0^2 = \frac{L}{4 \gamma^2} + D^2
\end{align*}
The result of Lemma~\ref{lem:IntersectingSpreadingPulse} gives us the
...
\begin{align}
e_3 - e_2
  & = \frac{L}{v}(1-\frac{1}{\gamma}) + \frac{ -\frac{Lv}{2\gamma} - \frac{Lv}{2\gamma} }{c^2-v^2} \nonumber \\
  & = \frac{L}{v}(1-\frac{1}{\gamma}) - \frac{Lv}{\gamma(c^2-v^2)} & & \text{algebra} \nonumber \\
  & = \frac{L}{c} \left[ \frac{1}{\beta}(1-\frac{1}{\gamma}) - \frac{\beta c^2}{\gamma(c^2-v^2)} \right] & & \text{replace $v$ with $\beta c$ and algebra} \nonumber \\
  & = \frac{L}{c} \left[ \frac{1}{\beta}(1-\frac{1}{\gamma}) - \beta \gamma \right] & & \text{$c^2 / (c^2 - v^2) = \gamma^2$, and a little algebra} \label{eqn:Bdeltatime3minus2} \\
\end{align}
...
\begin{align*}
\frac{1}{\gamma}(e_3 - e_2) & = -(q_3 - q_2) & & \text{what we want to show} \\
\frac{L}{c\gamma} \left[ \frac{1}{\beta}(1-\frac{1}{\gamma}) - \beta \gamma \right] & = - \frac{L}{\beta c} (1 - \frac{1}{\gamma}) & & \text{Eqns.~\eqref{eqn:Bdeltatime3minus2} and~\eqref{eqn:pulsesenddiff2}} \\
(1-\frac{1}{\gamma}) - \beta^2 \gamma & = -\gamma (1 - \frac{1}{\gamma}) & & \text{multiply both sides by $\gamma \beta c/L$} \\
(1-\frac{1}{\gamma}) + \gamma (1 - \frac{1}{\gamma}) & = \beta^2 \gamma & & \text{Add same expressions to both sides to rearrange terms} \\
\frac{(\gamma + 1)(\gamma - 1)}{\gamma} & = \beta^2 \gamma & & \text{algebra on left-hand side} \\
\frac{\gamma^2 - 1}{\gamma} & = \beta^2 \gamma & & \text{algebra on left-hand side} \\
1 - \frac{1}{\gamma^2} & = \beta^2 & & \text{divide both side by $\gamma$, algebra} \\
1 - (1-\beta^2) & = \beta^2 & & \text{defn. of $\gamma$.  This is true!}
\end{align*}
All multiplications, divisions, and cancellations avoided multiplying
or dividing by 0.  Therefore, since the final equation is true, so are
all earlier ones, including the first that we wanted to prove.
...
The only differences between Scenario 4 and Scenario 3
are that rod $R$ and $A$ are moving parallel to the $x$ direction
