...
Figure~\ref{fig:compare-d_E-to-d_A} compares the values of $d_E$ and
$d_A$ when $E$ and $A$ receive pulse 3 at the same time and place, for
$\beta_R = 0$, and all values of $\beta_S$ from 0.001 up to 0.999.
$L=1$ light-second, and $D=1000$ light-seconds.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\textwidth]{scen2b-where-E-starts-to-receive-pulse-3-at-A-location-D-1000.0-beta_R-0.000.pdf}
	\caption{Comparing $d_E$ to $d_A$ when $E$ and $A$ receive pulse 3 at the same time and place}
	\label{fig:compare-d_E-to-d_A}
\end{figure}

Let us examine the curves one at a time.  The curve labeled $Z/D$
shows that value of $Z$ that, for the given value of $\beta_S$, leads
to $E$ and $A$ being at the same place when they both receive the
pulse for Event 3.  It appears to be a straight line, suggesting that
either $Z = -\beta_S D$, or quite close to it.

It is not exactly that, as can be seen by examining the curve labeled
$Z+(beta*D)$, which shows that $Z$ is always lower than $-\beta_SD$,
but never by more than 0.5.

The top curve with the stars labeling it are two curves that are so
close that one overlaps the other at this scale: the curves labeled
\textbf{d\_E\_restclock} and \textbf{d\_A\_restclock}.  The curve with
label beginning \textbf{1000} shows that $d_E$ and $d_A$ are not equal,
but they never differ by more than one about 1 in 4000.
...
\\ \cline{1-3}
$(a \oplus b) \oplus c = a \oplus (b \oplus c)$ & yes & no (TODO: counterexample)
\\ \cline{1-3}
$a \oplus 0 = 0 \oplus a = a$ & yes & yes
\\ \cline{1-3}
$a \oplus (-b) = a \ominus b$ & yes & TODO
\\ \cline{1-3}
$a \ominus (-b) = a \oplus b$ & yes & TODO
\\ \cline{1-3}
$a \ominus (-b) = a \oplus b$ & yes & TODO
\\ \cline{1-3}
$0 \ominus b = -b$ & yes & yes
\\ \cline{1-3}
$a \ominus b = - (b \ominus a)$ & yes & TODO
\\ \cline{1-3}
\end{tabularx}
