...
\begin{align}
T_C
  & = \frac{\gamma_B}{\gamma_C} T_B (1 + \frac{\gamma_C^2}{c^2} \left[ (v_C^2 - \vect{v}_B \cdot \vect{v}_C) + \sqrt{(v_C^2 - \vect{v}_B \cdot \vect{v}_C)^2  + (1-\beta_C^2)c^2 |\vect{v}_C - \vect{v}_B|^2} \right] ) \label{eqn:scen2b-TC}
\end{align}

Let $r$ be the 3-d relativistic velocity difference
$\vect{v}_C \ominus \vect{v}_B$ given by
Equation~\eqref{eqn:relVelocitySubComponents} in Appendix
~\ref{app:3drelvelocityadd}.  Then while I have not yet proved that
$T_C = D_r T_B$ is equal to Equation~\eqref{eqn:scen2b-TC} for all
$\vect{v}_A$, $\vect{v}_B$, I have written a simple program that
demonstrates those equations give the same result for many randomly
selected pairs of values for $\vect{v}_A$, $\vect{v}_B$, so it is highly
likely those two equations always give the same result.
