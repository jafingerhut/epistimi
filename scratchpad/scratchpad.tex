...
relative to the ether, that $B$ experienced duration dilation,
i.e. its clock physically ran slower than $A$'s by a factor of
...
speed $c$ relative to $A$.

Everyone with this knowledge would be able to deduce that $B$'s clock
is running $\gamma$ times slower than full speed.  Also, that that
...
Further, relative to $B$, light would propagate isotropically relative
to the ether at constant speed $c$, and thus does {\em not} propagate
at the same speed in all directions relative to $B$.
...
\begin{align*}
  l_A(n, t_A) & = \text{position of $B$ when emitted} \\
            &   + \text{distance traveled after emission} \\
  l_A(n, t_A) & = x_B(t_A(n)) + (t_A - t_A(n)) c & & \text{for any time $t_A \geq t_A(n)$} \\
  l_A(n, t_A) & = v_B t_A(n) + (t_A - t_A(n)) c & & \text{substitute Eqn.~\eqref{eqn:scen2posB}} \\
  l_A(n, t_A) & = -(c - v_B) t_A(n) + t_A c & & \text{algebra} \\
  l_A(n, t_A) & = -(c - v_B) \gamma_B nT + t_A c & & \text{substitute Eqn.~\eqref{eqn:scen2timeA}}
\end{align*}
