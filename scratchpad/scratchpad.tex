...
Note: These equations can be derived from the Lorentz transformation
equations.  See
Appendices~\ref{app:3d-derive-relativistic-velocity-addition-eqns}
and~\ref{app:3d-derive-relativistic-velocity-subtraction-eqns}.
...
Note: Unless one first demonstrates a {\em different} way to derive
these transform equations (e.g. perhaps in the way that Voigt and
Lorentz did), this section demonstrates that these equations depend
upon Einstein's postulates.  That is, if you wish to investigate
theories that do not conform to Einstein's postulates, then you
{\em should not} use these equations without first deriving them in
some other way, since doing so could easily lead to incorrect results.
...
\begin{itemize}
\item \homogeneity: Space is homogenous, i.e. there are no places
  where things behave differently than they do in other places,
  because of the place alone.  Similarly, time is homogenous.
\item \linearity: If an entity is in constant velocity motion in one
  inertial frame, the transformation equations must map those to
  constant velocity motion in all other inertial frames.
\end{itemize}

\begin{lemma}
Let a transformation from inertial frame $S$ to frame $S'$ be
specified as a function from events $(t, x)$ to $(t', x')$.
If the transformation satisfies these constraints:
\begin{itemize}
\item \homogeneity
\item \linearity
\item It must take all points $x = \pm ct$ and transform them to
  points $x' = \pm ct'$ (this is another way to write \esol).
\end{itemize}
then we must have:
\begin{equation}
...
Define $Q$ and $Q'$ as:
\begin{align}
Q(x,t) = c^2 t^2 - x^2 \label{eqn:defnq} \\
Q'(x,t) = c^2 t'^2 - x'^2 \label{eqn:defnqprime}
\end{align}
...
satisfying $Q(x,t)=0$ and transform them to points satisfying
$Q'(x,t)=0$.
...
\begin{align}
x' & = Ax + Bt \label{eqn:defnxprime} \\
t' & = Dx + Et \label{eqn:defntprime}
\end{align}
where $A$, $B$, $D$, and $E$ may depend on $v$ and other constants.
They must not depend upon $x$ and $t$, because that would violate
\homogeneity.

Substitute Equations~\eqref{eqn:defnxprime} and~\eqref{eqn:defntprime}
into Equation~\eqref{eqn:defnqprime} to get:
\begin{align}
Q'(x,t)
  & = c^2 (Dx + Et)^2 - (Ax+Bt)^2 \nonumber \\
...
Q'(x,t)
  ...
  & = -a Q(x,t) & & \text{Eqn.~\eqref{eqn:defnq}}
\end{align*}
