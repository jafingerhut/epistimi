...
some distance apart.  We will say $R$ is to the left of $S$ for the
...
length.  There is another person $B$ at rest relative to $S$ at a
distance $D$ from $S$'s center, in the same direction $U$ from $S$'s
center.

There are 3 events of interest, shown in Figure~\ref{fig:scenario3-events}:
\begin{itemize}
\item Event 1: The right end of $R$ meets the left end of $S$.
\item Event 2: The right end of $R$ meets the right end of $S$.
\item Event 3: The left end of $R$ meets the left end of $S$.
\end{itemize}
...
Each of these events causes a radio pulse to be emitted from the right
or left end of rod $R$, when the event is detected locally by some
...
In $A$'s frame, the sending times of the pulses are:
\begin{align*}
...
interval between $B$ receiveing the pulses, we find that Event 3's
pulse reaches $B$ before Event 2's pulse, because in $B$'s frame, $S$
...
I believe that event 1 and 2 are timelike separated, because an object
with mass traveling at speed faster than $v$ relative to $A$, but less
than $c$, in the same direction as $S$'s velocity can move from event
1 to the place where event 2 occurs, before event 2 occurs.

Similarly event 1 and 3 are timelike separated.

Events 2 and 3 are spacelike separated, because it would require
faster-than-light travel in $A$'s frame to move from the right end of
$R$ starting when event 2 occurs, to the left end of $R$ by the time
event 3 occurs.  According to Special Relativity, there exist at least
some pairs of observers that could observe events 2 or 3 in different
orders from each other.

TODO: Is this all correct?

TODO: How can we analyze what $B$ measures, in $A$'s frame?
...
TODO: Give a lot of details for the case of $A$ at rest with respect
to ether, and speed of light propagates isotropoically at speed $c$.
Calculate the position along the rod $R$ where the light wavefronts
from the event 2 and event 3 pulses reach an observer at rest relative
to $A$ at the same time, with calculations for all values of $0 < v <
c$ of rod $S$.  The formula shows that this point is close to the
midpoint of $R$ when $v$ is close to 0, and the point is close to the
left end of $R$ when $v$ is close to $c$.  Show a figure with the X-Y
plane and the shape of the curve which is all points where event 2 and
event 3's pulses reach an at-rest observer at that position, at the
same time as each other.  The shape of this set of locations is a
hyperbola of all points where $d_r - d_l = constant$, where $d_r$ is
the distance from right end of $R$ to the point, and $d_l$ is the
distance from the left end of $R$ to the point.  Show the hyperbola
shapes varying for different values of the constant, and point out the
extreme case along the line of the rod to the right and left, where
the difference between the times of arriving pulses from event 2 and 3
is maximum.

TODO: Next, consider an observer $o$ moving with a velocity vector
$v_o$ starting at position $r_o$ at the time when the event 2 light
pulse is transmitted, where both $r_o$ and $v_o$ lie in the plane of
$A$ and the rods.  For a fixed value of $v_o$, how does that change
the graphs drawn above (which are the graphs for $v_o=0$)?
...
event 2 at the right end of $R$.  Similarly, there are many locations
that will receive the event 3 pulse first, e.g. at the location of
event 3 at the left end of $R$.  There are also many locations that
...
\begin{align*}
e_2 & = q_2 + d_r/c & & \text{$d_r$ is distance between right end of $R$ and $(x,y)$} \\
e_3 & = q_3 + d_l/c & & \text{$d_r$ is distance between left end of $R$ and $(x,y)$}
\end{align*}
...
\begin{align}
e_3 - e_2 & = k \nonumber \\
(q_3 + \frac{d_l}{c}) - (q_2 + \frac{d_r}{c}) & = k & & \text{defns. of $e_2, e_3$} \nonumber \\
d_l - d_r & = c (k - (q_3 - q_2)) & & \text{algebra} \label{eqn:pulserecvdiff1}
\end{align}
...
q_3 - q_2
  ...
  & = \frac{L}{\beta c} (1 - \sqrt{ 1 - \beta^2 }) & & \text{defns. of $\beta, \gamma$} \nonumber \\
  & = \frac{L}{\beta c} (1 - \sqrt{ 1 - \beta^2 }) \frac{1 + \sqrt{1-\beta^2}}{1 + \sqrt{1-\beta^2}} & & \text{multiply by a complicated-looking 1} \nonumber \\
  & = \left( \frac{L}{\beta c} \right) \frac{1 - (1 - \beta^2)}{1 + \sqrt{1-\beta^2}} & & \text{algebra} \nonumber \\
  & = \left( \frac{L}{c} \right) \frac{\beta}{1 + \sqrt{1-\beta^2}} & & \text{algebra} \label{eqn:pulsesenddiff}
\end{align}
The funny multiplying by 1 step was done because the resulting formula
below is easier to understand its behavior for values of $\beta$ near 0:
\begin{align*}
\frac{\beta}{1 + \sqrt{1-\beta^2}}
\end{align*}
When $\beta$ approaches 0, this value approaches $0/2 = 0$.
When $\beta$ approaches 1, this value approaches $1/1 = 1$.
...
\begin{align}
d_l - d_r
  & = c \left( k - \left( \frac{L}{c} \right) \frac{\beta}{1 + \sqrt{1-\beta^2}} \right) \nonumber \\
  & = ck - \frac{L \beta}{1 + \sqrt{1-\beta^2}}
\end{align}
If we restrict our attention to situations where $L$, $\beta$, and $k$
are given constants, this equation is of the form $d_l - d_r$ equal to
a constant.  The shape of all such curves is a hyperbola.
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{pulse-receive-deltas-L-1.0-beta-0.50.pdf}
...
earlier than pulse 3, all of these locations are further from the
right end of $R$ than from the left end.
...
arrives 0.27 {\em earlier} than pulse 2 (emitted earlier).

For the line $y=-D$ where $A$ and $B$ are (this is far down below the
bounds of the figure), $A$ and $B$ are both very far from the line
$k=-0.27$ when they receive the pulses.

TODO: Is there any way to analyze this scenario in $R$'s frame such
that $B$ receives event 3's pulse significantly before event 2's
pulse?  I do not see how, without putting $B$ quite far to the left of
$A$.  I believe that according to Special Relativity, $B$ receives the
pulse from event 3 significantly before the pulse from event 2, even
if it is very near $A$ when the pulses are received.

TODO: I plan to repeat this analysis in $R$'s frame for an observer
moving at a constant speed $v$ in the $x$ direction, just in case that
shows significantly different results.  It would surprise me greatly
if that resulted in $B$ receiving the pulses in the opposite order
from $A$, with a similar time interval between them as $A$ measures.
(Note: Lemma\ref{lem:IntersectingSpreadingPulse} is some work towards
this goal.)
...
sound wave) is emitted from a position $\vect{s} = (s_x, s_y)$ in the
$XY$ plane at time $t_0$, propagating in all directions at speed $c$.

An observer $O$ is moving at constant velocity $v$ in the $x$
direction (i.e. no motion in the $y$ or $z$ components) starting at
position $\vect{s} + \vect{r}$ at time $t_1$, where
$\vect{r} = (x_0,y_0)$.

If $|v| < c$, then there is exactly one time $t_r \geq t_0$ that $O$
encounters the pulse signal, given by:
\begin{equation}
t_r = t_0 + \frac{v{x_0}' + \sqrt{c^2{r'}^2 - v^2y_0^2}}{c^2-v^2}
\end{equation}
where:
\begin{align*}
x_0' = x_0 + v(t_0 - t_1) \\
{r'}^2 = {{x_0}'}^2 + y_0^2
\end{align*}
$t_0, t_1, v, x_0, y_0, c$ can be arbitrary real numbers subject to
the restriction $|v| < c$.
If you choose $t_1 > t_0$, it is assumed that $O$'s constant velocity
movement extends back to $t_0$ or earlier, and $O$ reaches the point
$\vect{s}+\vect{r}$ at time $t_1$.
\end{lemma}
The position of $O$ at time $t$ for
Lemma~\ref{lem:IntersectingSpreadingPulse} is given by:
\begin{equation}
x_O(t) = s_x + x_0 + v(t-t_1), y_0(t)=s_y + y_0
\end{equation}
The proof of the above is to use the proof of
Lemma~\ref{lem:IntersectingSpreadingPulse2} below, with $x_0'$ in
place of $x_0$.

The result for an observer at rest very simple, but we mention it here
anyway.
\begin{corollary}
A spherically spreading ``pulse'' (e.g. a light pulse, or a brief
sound wave) is emitted from a position $\vect{s}$ in the $XY$ plane at
time $t_0$, propagating in all directions at speed $c$.

An observer $O$ is at rest relative to where the pulse is emitted from,
at position $\vect{s} + \vect{r}$, where $\vect{r} = (x_0,y_0)$.

The time $t_r \geq t_0$ that $O$ encounters the pulse signal is given
by:
\begin{equation}
t_r = t_0 + \frac{r}{c} = t_0 + \frac{\sqrt{ x_0^2 + y_0^2 }}{c}
\end{equation}
$t_0, x_0, y_0, c$ can be arbitrary real numbers subject to the
restriction $c > 0$.
\end{corollary}

\begin{lemma}
\label{lem:IntersectingSpreadingPulse2}
Exactly the same scenario as described in
Lemma~\ref{lem:IntersectingSpreadingPulse}, except that the initial
position for observer $O$ must be given at time $t_1=t_0$, the same
time that the pulse is emitted.

The observer encounters the pulse signal at time $t_r$ given by:
\begin{equation}
t_r = t_0 + \frac{vx_0 + \sqrt{c^2r^2 - v^2y_0^2}}{c^2-v^2}
\end{equation}
$t_0, v, x_0, y_0, c$ can be arbitrary real numbers subject to
the restriction $|v| < c$.
\end{lemma}
The position of $O$ at time $t$ for the lemma above is given by:
\begin{equation}
x_O(t) = s_x + x_0 + v(t-t_0), y_0(t)=s_y + y_0
\end{equation}
Lemma~\ref{lem:IntersectingSpreadingPulse2} is simply a slight
generalization of Lemma~\ref{lem:IntersectingSpreadingPulse3}, but
with the light pulse starting point translated from the origin to
$\vect{s}$, and the pulse being transmitted at a translated time $t_0$
instead of 0.
...
\begin{lemma}
\label{lem:IntersectingSpreadingPulse3}
A spherically spreading ``pulse'' (e.g. a light pulse, or a brief
sound wave) is emitted from the origin at time $t=0$, propagating in
all directions at speed $c$.

An observer $O$ is moving at constant velocity $v$ in the $x$
direction (i.e. no motion in the $y$ or $z$ components) starting at
position $\vect{r} = (x_0,y_0)$ at $t=0$.

If $|v| < c$, then there is exactly one time $t_r$ that $O$ encounters
the pulse signal, given by:
\begin{equation}
t_r = \frac{vx_0 + \sqrt{c^2r^2 - v^2y_0^2}}{c^2-v^2}
\end{equation}
$v, x_0, y_0, c$ can be arbitrary real numbers subject to the
restriction $|v| < c$.
\end{lemma}
Aside: In addition to the proof below, I have verified the equation
above with a handful of sets of values for $c, v, x_0, y_0$ and
verified that it gives a positive value of $t_r$ such that the
position of the observer at time $t_r$ is distance $ct_r$ from the
origin.
\begin{proof}
Since $O$ moves only in the $XY$ plane, we need only concern ourselves
with the circle of the wavefront in that plane, and when $O$
intersects with it.  The wavefront is spread along a circle with
points $(x,y)$ satisfying the equation:
\begin{equation}
x^2 + y^2 = c^2t^2 \label{eqn:circularwavefront}
\end{equation}
The position of observer $O$ at time $t$ is:
\begin{equation}
x_O(t) = x_0 + vt, y_0(t)=y_0
\end{equation}
$O$ receives the pulse when its position satisfies
Equation~\eqref{eqn:circularwavefront}.
\begin{align*}
(x_0 + vt)^2 + y_0^2 & = c^2t^2 \\
x_0^2 + 2x_0vt + v^2t^2 + y_0^2 & = c^2t^2 & & \text{algebra} \\
(v^2 - c^2)t^2 + 2x_0vt + (x_0^2 + r_0^2) & = 0 & & \text{collect like terms by powers of $t$}
\end{align*}
Use the quadratic equation, and let $r^2 = x_0^2 + y_0^2$ to reduce
the number of symbols a bit.  First evaluate the discriminant:
\begin{align*}
b^2 - 4ac
  & = (2x_0v)^2 - 4(v^2-c^2)(x_0^2+y_0^2) \\
  & = 4x_0^2v^2 - 4v^2x_0^2 - 4v^2y_0^2 + 4c^2x_0^2 + 4c^2y_0^2 \\
  & = 4 \left[ c^2(x_0^2 + y_0^2) - v^2y_0^2 \right] \\
  & = 4 \left[ c^2r^2 - v^2y_0^2 \right]
\end{align*}
The full solutions to the equation are:
\begin{align*}
t_r
  & = \frac{-2vx_0 \pm \sqrt{4 (c^2r^2 - v^2y_0^2) }}{2(v^2-c^2)} \\
  & = \frac{vx_0 \pm \sqrt{c^2r^2 - v^2y_0^2}}{c^2-v^2} & & \text{multiply numerator and denominator by $-1/2$}
\end{align*}
We want only a positive solution, since that is after the pulse is
emitted.  $|v| < c$, so the denominator is positive.  If $vx_0$ is
negative, the only way to get a positive solution is to use the
addition alternative among the two solutions.
\begin{align*}
t_r & = \frac{vx_0 + \sqrt{c^2r^2 - v^2y_0^2}}{c^2-v^2}
\end{align*}
\end{proof}
