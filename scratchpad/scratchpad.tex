...
pulse 2 (or before if $k$ is negative) the following equation must be
true:
\begin{align}
e_3 - e_2 & = k \nonumber \\
...
\begin{align}
d_l - d_r
  & = c (k - \left( \frac{L}{c} \right) \frac{\beta}{1 + \sqrt{1-\beta^2}}) \nonumber \\
...
$k=-0.27$ when the receive the pulses.

TODO: Is there any way to analyze this scenario in $A$ and $R$'s frame
such that $B$ receives event 3's pulse significantly before event 2's
pulse?  I do not see how, without putting $B$ quite far to the left of
$A$.

TODO: I plan to repeat this analysis in $A$ and $R$'s frame for an
observer moving at a constant speed $v$ in the $x$ direction, just in
case that shows significantly different results, but it would surprise
me greatly if that resulted in $B$ receiving the pulses in the
opposite order from $A$, with a similar time interval between them as
$A$ measures.
...
emitted.  $v < c$, so the denominator is positive.  The square root is
at least $cr$, which is greater than $|vx_0|$, so we should use the
