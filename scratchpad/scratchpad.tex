...
place when they both receive the pulse for Event 3.  It appears to be
a straight line, suggesting that either $Z = -\beta_S D$, or quite
...
The top curve with both stars and X's labeling it are two curves that
are so close that one overlaps the other at this scale: the curves
labeled \textbf{d\_E\_restclock} and \textbf{d\_A\_restclock}.  The
curve with label beginning \textbf{1000} shows that $d_E$ and $d_A$
are not equal, but they never differ by more than about 1 part in
4000.
